...
const int INF = 1e8;
int counter = 0;
const int threads_number = 2; // for buses
...
//для работы алгоритма Floyd-Warshall
    std::vector<std::vector<int>> distances; // здесь кратчайшие дистанции 
    std::vector<std::vector<int>> next; // отсуюда можно вывывести
...
std::vector<int> get_common_streets(int bus1, int bus2) {
    ...
    // Находим начальные остановки для каждого автобуса
    int start_stop_bus1 = stops_bus1[0];
    int start_stop_bus2 = stops_bus2[0];

    // Вызываем алгоритм Дейкстры для поиска кратчайших путей от начальных остановок
    std::vector<int> parent_bus1 = dijkstra(start_stop_bus1);
    std::vector<int> parent_bus2 = dijkstra(start_stop_bus2);

    // Находим общие улицы между маршрутами автобусов
    std::vector<int> common_streets;

    // Перебираем каждую остановку второго автобуса
    for (int stop : stops_bus2) {
        // Проверяем, принадлежит ли остановка маршруту первого автобуса
...
int main() {
  ...
  //for check after inserting a new crossroad and streets 
  ...
  for(int i = 0; i < myCity.buses.size(); ++i)
  {
      ...
      {
          std::cout << "duration from " << myCity.buses[i][0] << " to " << myCity.buses[i][j] << " - " << myCity.distances[i][myCity.buses[i][j]]<< "\n";
      }
      std::cout << "\n";
  }
